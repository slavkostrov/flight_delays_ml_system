...
def train_model(config: Config):
    ...
    with mlflow.start_run(description="flight_delay_model_evaluation") as active_run:
        ...
        train = train.limit(config.max_train_size)
...
def eval_model(config: Config):
    ...
    latest_pipeline_version = client.get_latest_versions(f"pipeline_{config.model_name}")[-1].version
    ...
    val_prod = current_prod_pipeline.transform(val.drop(val.features))
    val_latest = latest_pipeline.transform(val.drop(val.features))
    ...
    if NEW_MODEL_BETTER:
        ...
        client.transition_model_version_stage(name=f"pipeline_{config.model_name}", version=latest_pipeline_version,
...
def move_model_to_s3(config: Config):
    ...
    mlflow.pyfunc.load_model(f"models:/{config.model_name}/production", dst_path=f"{local_path}/model/")
    mlflow.pyfunc.load_model(f"models:/pipeline_{config.model_name}/production", dst_path=f"{local_path}/pipeline/")
